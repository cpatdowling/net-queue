...
step = float(0.01)
rates = 80
utilizationStats = np.zeros((rates,4))
averageWaitTime = np.zeros((rates,2))

paramsInst = parameters("/usr/lusers/cdowling/projects/net-queue/data/params/QNet3params.txt")
...
avgOut = ("/usr/lusers/cdowling/projects/net-queue/data/3bus/averageWait" + str(np.random.randint(1,100000)) + ".txt")
avgFile = open(avgOut, 'w')
utilizationOut = ("/usr/lusers/cdowling/projects/net-queue/data/3bus/utilization" + str(np.random.randint(1,100000)) + ".txt")
