...
rho_line_num = sys.argv[4]
rho_line = int(rho_line_num) # cuz I'm lazy

#row number for data files as sys.argv[4] and enter manually to making parallelize script easier

try:
    os.mkdir(workingPath + paramDir + "-sim-all")
except:
    pass
try:
    os.mkdir(workingPath + paramDir + "-sim-all/" + sys.argv[2] + "-" + rho_line_num)
except:
    pass
    
#pulling lambda from rho file
with open(workingPath + paramDir + "/" + paramDir + "lam.csv", 'r') as f:
    lines = f.readlines()
    rho_lambd = lines[rho_line].strip()

def write_param_file(paramDir):
    #read paramDir + nash.csv or so.csv
    ...
    with open(workingPath + paramDir + "/" + paramDir + sys.argv[2] + ".csv", 'r') as f:
        ...
        """
        probs = lines[rho_line + 1].strip().split(",")
    ...
    with open(workingPath + paramDir + "/" + paramDir + "params.csv", 'r') as f:
        ...
        #getting cooresponding game strategy
    ...
    #for each blockface       
    with open(workingPath + paramDir + "-sim-all/" + sys.argv[2] + "-" + rho_line_num + "/busparams.txt", 'w') as out:
        for header in paramDict.keys():
            if header == "lam":
                #out.write("EXOGENOUS_RATE = " + str(1.0 / paramDict["lam"]) + "\n")
                out.write("EXOGENOUS_RATE = " + str(1.0 / float(rho_lambd)) + "\n")
            elif header == "c":
...
params = parameters(workingPath + paramDir + "-sim-all/" + sys.argv[2] + "-" + rho_line_num + "/busparams.txt")
...
#Cost to park
Cp = float(simulationParams["Cp"]) * float(1.0 / float(rho_lambd))
#cost to wait
Cw = Cwait
...
avgOut = ("/home/chase/Dropbox/UWSDOT/tex/src_/" + paramDir + "-sim-all/" + sys.argv[2] + "-" + rho_line_num + "/averageWait" + str(np.random.randint(1,100000)) + ".txt")
avgFile = open(avgOut, 'w')
utilizationOut = ("/home/chase/Dropbox/UWSDOT/tex/src_/" + paramDir + "-sim-all/" + sys.argv[2] + "-" + rho_line_num +  "/utilization" + str(np.random.randint(1,100000)) + ".txt")
...
dataOut = ("/home/chase/Dropbox/UWSDOT/tex/src_/" + paramDir + "-sim-all/" + sys.argv[2] + "-" + rho_line_num +  "/stats" + str(np.random.randint(1,100000)) + ".txt")
