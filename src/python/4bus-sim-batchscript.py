...
step = float(0.01)
rates = 80
utilizationStats = np.zeros((rates,5))
averageWaitTime = np.zeros((rates,2))

paramsInst = parameters("/usr/lusers/cdowling/projects/net-queue/data/params/QNet4params.txt")
...
avgOut = ("/usr/lusers/cdowling/projects/net-queue/data/4bus/averageWait" + str(np.random.randint(1,100000)) + ".txt")
avgFile = open(avgOut, 'w')
utilizationOut = ("/usr/lusers/cdowling/projects/net-queue/data/4bus/utilization" + str(np.random.randint(1,100000)) + ".txt")
