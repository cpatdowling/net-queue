'''
    ...
    2016.14.03
    Code for generating example in Figure 2 of 'To observe or not to Observe'
    CDC 2016 paper
'''
...
if __name__ == "__main__":
    ...
    # set parameters
    # parameters for example in figure 2 of the paper
    # on street v off street example
    lam = 1/4.35; c = 30; mu = 1/120.0; Cp = 0.05; Cw = 0.05*30; R = 95.0; n = 100; 
    Coff=0.962 
    lamvals=[]
    rho_=np.linspace(0.1,0.9,36)
    lam_=np.linspace(0.025,0.225,36)
    lam = lam_[0];
    Co=3.85
 

    param_nam=['lam','c','mu','Cp','R','Co','n', 'Cw','Coff']
    param_val=[lam,c,mu,Cp, R, Co, n, Cw,Coff]
    #var=rho_
    #var_=lam_
    #constant added to all utilities
    x1=5
    var_=Co_
    par='cdcfig2'
    ...
    for lam in lam_:
        ...
        sopt_.append(sopt)
        swlf_.append([swlf+x1])
        nash_.append(nash)
        nwlf_.append([nwlf+x1])
        lamvals.append([lam])
    ...
    if plot:
        ...
        sfxs=['png','pdf']
        colors=['darkblue','darkgreen','darkred']
        plt.plot(var,swlf_,c=colors[0],lw=2.0, label='Social ')
        plt.plot(var,nwlf_,c=colors[1],lw=2.0, label='Nash ')
        lgd=ax.legend(bbox_to_anchor=(1.05, 1), loc=2, borderaxespad=0.)
        ...
        sopt_=np.asarray(sopt_)
        nash_=np.asarray(nash_)
        plt.plot(var,sopt_[:,0],c=colors[0],lw=2.0, label=r'$p_o$ Social')
        plt.plot(var,sopt_[:,1],c=colors[1],lw=2.0, label=r'$p_b$ Social')
        plt.plot(var,sopt_[:,2],c=colors[2],lw=2.0, label=r'$p_j$ Social')
        plt.plot(var,nash_[:,0],'^',c=colors[0],lw=2.0, label=r'$p_o$ Nash')
        plt.plot(var,nash_[:,1],'^',c=colors[1],lw=2.0, label=r'$p_b$ Nash')
        plt.plot(var,nash_[:,2],'^',c=colors[2],lw=2.0, label=r'$p_j$ Nash')
        lgd=ax.legend(bbox_to_anchor=(1.05, 1), loc=2, borderaxespad=0.)
